...
The close state transition, archive projection/create, Participant anonymization and transactional outbox append use the same PostgreSQL transaction and commit or roll back together. Socket publication occurs only after commit.
...
- Immutable SessionQuestion snapshot and role-authorized result projection: poll/quiz expose aggregate-only data; open-text exposes anonymous raw `{ text }`. Poll/quiz raw Submission rows remain only in governed source storage until BE-5.2/BE-5.3 purge. Persisted/legacy JSON is untrusted at the HTTP boundary and must be reconstructed through a nested strict allowlist; unknown root/question/result/option/response fields are dropped, while malformed required fields fail closed.
- No display-name/participant-token join, no participant identity lookup and no learner history entry point. This is projection-level de-identification: Submission may retain an FK to an anonymized Participant until governed purge, but normal archive queries must not reconstruct answer-to-person linkage.
- Teacher scope limited to owned Course; admin scope limited to support/management authority.
- Archive finalization replay returns the existing ArchivedResult without rebuilding or replacing its payload, while re-applying Participant anonymization in the same transaction.
- Archive list supports Course, LiveSession, status and inclusive `closedAt` range filters. Date bounds are offset-bearing ISO instants; ownership and all filters are applied before count/pagination, with deterministic `closedAt DESC, id DESC` ordering. Student access is denied and foreign detail remains existence-hiding.
...
- Course teacher creates a deletion request with an allowed reason; this does not delete data. An outstanding-request replay returns the first receipt and does not rewrite its reason or timestamp.
- Admin must authenticate, complete session-bound step-up and explicitly confirm the whole LiveSession archive deletion using the **same reason** stored on the Teacher request; Admin does not independently reclassify the request.
- Request ID and LiveSession ID are matched before disclosure: missing or cross-session identity returns the same existence-hiding 404. Reason mismatch returns stable 409 `CONFLICT` with `field:"reason"` before any answer/archive/event/outbox write.
- There is no pre-delete answer export or hidden backup. The operation is not reversible.
- Deletion and minimum tombstone/audit event use one locked, idempotent transaction or resumable job with stable status.
- A matching repeated confirm returns the canonical existing deletion result; a conflicting-reason replay returns the same non-destructive 409. Neither path recreates or partially deletes data, and no schema migration is required for this rule.
