...
- [x] BE-5.2.5 建立 retry/restart tests
...
> - **BE-5.2.5** — `RetentionScheduler`／`ManifestExportScheduler` 的 process-restart lifecycle 專屬 unit tests：`onModuleInit()` 於 re-init 前清掉既有 interval（restart-safe，不泄漏第二個 wake source），加 3 個 fake-timer 測試 per scheduler 對齊 `tasks/lessons.md:247` tripwire（restart 重做 startup scan 且 interval 恰一、duplicate init 不產生 duplicate wake source、repeated destroy 至多 cleanup 一次）。backend `aaa115b`；guarded build＋tests 層完成。**不宣稱** production multi-replica／真實 process crash-restart runtime 證據（仍待 Checkpoint G step 5 staging gate）。
...
>
> **未勾項與未解除 blockers（維持 BE-5 APPROVAL WITHHELD）：** BE-5.2.5 之 process restart recovery **unit test** 已由 `aaa115b` 完成（見上方 BE-5.2.5 bullet）；**production 端仍未證**：migration apply、scheduler multi-replica／real process crash-restart runtime、staging dry-run、可部署 dashboard＋Alertmanager routing/on-call、capacity/W1–W8、production purge/object-store/restore——仍待 BE-5 final reconciliation 與 OPS-1。
...
| BE-5.2.5 | EVIDENCE AVAILABLE—unit 已證、provenance/ops 待審 | guarded retry/concurrency、local-provider lease tests、**process-restart lifecycle unit tests**（`aaa115b`；schedulers `onModuleInit` re-init restart-safe、fake-timer 證明 restart/destroy 至多一次 wake source） | MinIO spec/相關變更未完整 commit；filtered no-resurrection run 有 8 個 name-filter skips；**無 production crash/rolling/multi-replica restart runtime evidence** |
...
| Scheduler overlap/shutdown/restart | PARTIAL—unit 已證、runtime PARTIAL        | in-process overlap guard、shutdown drain、歷史 6 tests、**process-restart lifecycle unit tests**（`aaa115b`）；sandbox 記錄 MinIO/Postgres restart | 無 production process crash/rolling/multi-replica restart runtime evidence（unit 層 restart-safe 已證；runtime 仍待 Checkpoint G step 5 staging gate）                                                                                                          |
