...
> [!important] Phase B scenario override（2026-08-23）
> 早期 BDD 中 R-F5-5 的 admin/teacher-only account scope 與 anonymous-only learner story 已由 Phase B backend decision supersede。新增 current scenario contract：admin 建立 student（無公開 self-registration）；teacher/admin 加選與移除 `CourseEnrollment`；active enrolled student 以 cookie join/submit；student owner/admin paths 必須被拒且 `canCreateCourse=false`；匿名 session-code + participant-token fallback 保留。open_text results 在 open/closed 都只含匿名 plain-text responses，realtime student 只收 participant-safe projection、不收 teacher-only counts；student 沒有歷史結果入口。
> 本 override 描述需求行為，不把程式碼存在誤當成 B3/full regression evidence。

> [!info] 目的
> 將 [[P0 核心需求基線]] P0-01～P0-05 與 [[題目領域契約]] P0-04 的功能需求轉成可執行 BDD 場景（Given-When-Then）。本檔只涵蓋**功能行為視角**（單人正常流程與非負載邊界）；**負載與效能門檻視角**由 [[MVP 效能需求 BDD 場景]] 涵蓋。兩檔在競態、idempotency 與重連主題交叉引用，不重複定義門檻數值。

> [!note] 補自 [[需求蒐集]] 未被基線接手的部分
> US-F16（`can_create_course` 開課授權）與 US-F17（結果統計圖呈現）的規則在 [[需求蒐集]] 的 US-01／US-02／US-03 與 BR-01、BR-08、BR-16、BR-17 中已確認，但未被 [[P0 核心需求基線]] 或 [[題目領域契約]] 重述，故於本檔補齊為 BDD 場景。CLI／Agent 建題流程另見 [[CLI Agent 建題 BDD 場景]]。

> [!note] 驗收勾選語意
> 本檔 `[ ]` 代表 BDD 場景已由 [[P0 核心需求基線]] 與 [[題目領域契約]] 規則推導起草，待 M2 系統設計與 M3/M4 實作測試驗證後才標記 `[x]`。
...
# P0-01 Course 與 LiveSession 生命週期
...
- R-F0-1　只有帶 `can_create_course` 旗標的帳號可建立課程；未授權帳號嘗試建立課程時被拒。
- R-F0-2　被授權帳號建立課程後，Course 狀態為 `draft`，且建立者自動成為該 Course 唯一老師（一門課一位老師，建立者）。
- R-F0-3　Course 為持久容器，狀態為 `draft` 或 `archived`，不使用 `active`／`closed` 表示授課狀態。
- R-F0-4　MVP 不支援共同授課、老師指派或所有權移轉。
- R-F0-5　只有 `draft` Course 可建立 LiveSession、編輯課程資料與題目。
- R-F0-6　互動題目歸屬課程，只有該課老師能建立與修改。
...
- R-F1-1　Course 為持久容器，狀態為 `draft` 或 `archived`，不使用 `active`／`closed` 表示授課狀態。
- R-F1-2　只有 `draft` Course 可建立 LiveSession、編輯課程資料與題目。
- R-F1-3　Course 只有在不存在 `waiting` 或 `active` LiveSession 時才能封存。
- R-F1-4　`archived` 不可恢復、不可編輯、不可再建立場次，但保留歷史場次與結果查詢。
- R-F1-5　MVP 不提供 Course 刪除。
...
- R-F2-2　LiveSession 建立時進入 `waiting`，並取得自己的 session code。
- R-F2-3　`waiting → active` 時依當下選定題目及順序建立 SessionQuestion 不可變快照。
- R-F2-4　`active` 不代表所有題目自動開放；SessionQuestion 初始均為 `not_open`，由老師逐題控制。
- R-F2-5　`cancelled` 只適用於尚未 active 且沒有任何 Submission 的場次。
- R-F2-6　`closed` 與 `cancelled` 都是不可逆終止狀態，之後不可加入、重連或作答。
- R-F2-7　LiveSession 建立滿 8 小時仍未終止時系統自動 closed（負載視角見 [[MVP 效能需求 BDD 場景#US-P7 場次逾 8 小時自動關閉（W8）]]）。
...
- R-F3-1　每個 LiveSession 產生一組新的 8 碼 code，字元集為大寫英文字母與數字，排除 `O/0`、`I/1` 等混淆字元。
- R-F3-2　使用者輸入時不分大小寫。
- R-F3-3　Code 只在所屬 LiveSession 的 `waiting` 與 `active` 期間有效；closed、cancelled 或滿 8 小時時立即失效。
- R-F3-4　舊 code 不得指向後續新場次，也不得重新啟用。
- R-F3-5　Code 只用於找到及加入 LiveSession，不作為 Participant 身分或 CLI credential。
...
- R-F4-1　QuestionDefinition 歸屬 Course，可供同一 Course 的多個 LiveSession 重用。
- R-F4-2　老師建立 LiveSession 時可選擇題目子集合及順序。
- R-F4-3　場次 `waiting → active` 時建立該場 SessionQuestion 不可變快照。
- R-F4-4　LiveSession active 後修改 QuestionDefinition 不影響進行中或歷史場次。
- R-F4-5　SessionQuestion 逐題狀態為 `not_open → open → closed`，同一時間最多一題 `open`。
- R-F4-6　SessionQuestion closed 後不得在同一 LiveSession 重開；MVP 不支援 active 後新增題目。
- R-F4-7　同一 QuestionDefinition 可在下一個 LiveSession 再次使用，從 `not_open` 重新開始。
...
- R-F5-1　部署時提供一次性 bootstrap 流程建立第一位系統管理員；成功後自動失效，不得再次建立。
- R-F5-2　後續帳號由有效系統管理員建立，建立時簽發一次性臨時密碼。
- R-F5-3　使用者首次登入必須設定自己的新密碼後才能使用其他功能。
- R-F5-4　管理員無法讀回使用者後續設定的密碼。
- R-F5-5　MVP 只建立系統管理員與老師帳號，不建立學員帳號。
- R-F5-6　MVP 不提供帳號硬刪、Email 自助重設、SSO 或 MFA。
...
- R-F6-1　使用者以獨立 DB 帳號登入，不整合 SSO。
- R-F6-2　Web Session 由 Secure、HttpOnly、SameSite cookie 保護。
- R-F6-3　閒置 30 分鐘或自登入起滿 8 小時，Session 失效並要求重新登入。
- R-F6-4　使用者可登出目前 Session；登出後瀏覽器返回不得重新顯示受保護內容。
- R-F6-5　停用帳號、管理員重設密碼或使用者成功變更密碼時，所有既有 Web Session 立即失效。
- R-F6-6　建立／輪替 CLI key 及管理員高風險操作，要求最近 10 分鐘完成密碼 step-up 驗證。
- R-F6-7　`active` 帳號可登入；`disabled` 帳號收到通用登入失敗且不得建立 Session。
...
- R-F7-1　密碼長度 12～128 個 Unicode 字元，允許 passphrase。
- R-F7-2　不要求固定大小寫、數字、符號組合，也不要求定期更換。
- R-F7-3　不得使用平台列為常見或已知外洩的密碼。
- R-F7-4　密碼不得明文保存；實際雜湊演算法與參數由 M2 在 [[技術棧]] 定案。
- R-F7-5　使用者變更密碼時須先驗證目前密碼。
- R-F7-6　MVP 不提供 Email 自助重設；忘記密碼由管理員簽發新的一次性臨時密碼，下次登入強制改密碼。
- R-F7-7　登入失敗採帳號與來源雙重 rate limit，回傳不洩露帳號是否存在的通用錯誤，不得因大量失敗永久鎖死帳號。
...
- R-F8-1　`active` 帳號可依角色登入並執行授權功能。
- R-F8-2　停用帳號時既有 Web Session、CLI credential 與未使用 token 立即失效。
- R-F8-3　停用不刪除 Course、LiveSession、ArchivedResult 或稽核資料。
- R-F8-4　CLI key security lock 只阻擋 CLI credential 建立／啟用／輪替，不等同停用 Web 帳號；帳號停用則同時阻擋 Web 與 CLI。
- R-F8-5　停用後可由管理員恢復為 `active`。
- R-F8-6　MVP 不硬刪帳號。
...
# P0-04 題目領域契約（題型語意與共用 validation）
...
- R-F9-5　第一版不支援 Markdown、HTML、附件、圖片或其他 rich content。
- R-F9-6　Question ID 由平台指定且不可變；Web／CLI 均不得自訂正式 ID。
...
- R-F10-1　題幹與 option text 先去前後空白再檢查空值及長度。
- R-F10-2　重複 option 以 Unicode 正規化、trim、合併連續空白及忽略大小寫後判斷；正規化後重複為阻擋 error。
- R-F10-3　批次建立每批 1～50 題；`client_ref` 在單次 payload 內唯一。
- R-F10-4　All-or-nothing：任一 error 均不得建立部分題目。
- R-F10-5　Validation 一次回報所有可辨識的 error 與 warning。
- R-F10-6　同一 Course 可有相同題幹；validation 可提「可能重複題目」warning 但不阻擋或自動合併。
- R-F10-7　Error／warning 至少回傳穩定語言無關 code、可定位題目與欄位的 path、人類訊息、是否阻擋、使用者可採取的下一步。
...
- R-F11-1　Validation 成功後的 preview 須完整呈現 Course 名稱與 ID、題目順序、內容、正確答案及 warning。
- R-F11-2　正式建立前必須由老師明確確認；payload 改變即重新 validation 與確認。
- R-F11-3　正式建立採相同 validation 結果、授權重查、transaction 與 idempotency 規則。
- R-F11-4　成功題目依預覽順序附加到 Course 當下末端。
- R-F11-5　Web 單題 CRUD 不受每批最多 50 題限制。
- R-F11-6　Course archived 後不得修改題目。
...
# P0-05 匿名 Participant、重連、提交與競態（功能視角）

> [!note] 與效能檔的分工
> 以下為**功能行為視角**（單人正常流程與非負載邊界）。**負載與門檻視角**（300 人集中提交、p95 延遲、同時斷線重連等）見 [[MVP 效能需求 BDD 場景]]，本檔不重複門檻數值。
...
- R-F12-1　學員以 session code 與自填顯示名加入 LiveSession。
- R-F12-2　加入成功後由伺服器簽發只對該 LiveSession 有效的 opaque participant token。
- R-F12-3　Token 可由同一 browser 保存並用於重連，但不得用於其他 LiveSession，也不建立跨場追蹤身分。
- R-F12-4　清除 browser 資料、改用其他 browser 或裝置後，平台不保證識別為同一 participant。
- R-F12-5　顯示名去前後空白後須為 1～40 個安全可顯示 Unicode 字元，不得含換行、控制字元或顯示方向控制符。
- R-F12-6　同一 LiveSession 允許重複顯示名；顯示名只供 UI 展示，不作身份、防重複或授權依據。
...
- R-F13-1　有效 participant token 重連後取得：LiveSession 最新狀態、目前 SessionQuestion 及狀態、自己是否已提交目前題目、依 vote-to-reveal 有權看到的最新彙總。
- R-F13-2　瀏覽器快取或 WebSocket 事件不是權威資料；斷線後以伺服器回傳的最新狀態為準。
- R-F13-3　LiveSession closed／cancelled 或滿 8 小時後不得重連。
- R-F13-4　重連不建立第二個 participant，已答題目不建立第二筆有效 Submission。
...
- R-F14-2　第一次成功提交後不得修改答案。
- R-F14-3　同一 idempotency key 的安全重送回傳第一次提交結果，不新增第二筆 Submission。
- R-F14-4　同一 participant 對同一題以不同答案再次提交回傳衝突，不覆寫第一次答案。
- R-F14-5　多分頁或併發請求同時提交時，最多一筆成功。
- R-F14-6　只有 LiveSession `active` 且 SessionQuestion `open` 時可提交。
...
- R-F15-1　以伺服器提交順序為唯一權威，不採 client device time 或 WebSocket 到達先後。
- R-F15-2　Submission 先成功提交後題目 closed → 答案保留並納入結果。
- R-F15-3　題目先成功 closed 後 Submission 提交 → 答案拒絕，不納入結果。
- R-F15-4　Client 已送出但未取得回應時，以原 idempotency key 取回第一次權威結果。
- R-F15-5　WebSocket 顯示已關題但權威查詢顯示 Submission 已先提交時，以權威查詢為準。
- R-F15-6　LiveSession 自動 closed 與 Submission 同時發生時，同樣依伺服器提交順序判定。
...
> [!note] 規則來源
> [[需求蒐集]] BR-01、US-01（AC-01-02、AC-01-03）、US-02（AC-02-01）。`can_create_course` 是帳號層級旗標，因為課程建立前不存在可指派的標的；該帳號建立課程後自動成為該課老師。此規則在 [[P0 核心需求基線]] 的帳號生命週期中未重述，於本檔補齊。

### Rules

- R-F16-1　`can_create_course` 為帳號層級旗標，由系統管理員在帳號上切換。
- R-F16-2　只有帶 `can_create_course` 旗標的帳號可建立課程；未授權帳號嘗試建立課程時被拒。
- R-F16-3　被授權帳號建立課程後自動成為該課唯一老師（一門課一位老師，建立者）。
- R-F16-4　互動題目歸屬課程，只有該課老師能建立與修改。
- R-F16-5　移除 `can_create_course` 時，相關 CLI credential 與未使用 token 立即失效，既有課程與題目不刪除（對應 [[P0 核心需求基線#角色與帳號狀態]] 的 credential 失效規則）。
...
# boundary — 移除旗標使 CLI credential 失效但保留課程
Scenario: 移除可開課旗標使 CLI credential 失效但保留課程
  假定 一位帶 can_create_course 的老師有 CLI credential 與既有課程
  當 系統管理員移除該帳號的 can_create_course
  那麼 相關 CLI credential 與未使用 validation token 立即失效
  而且既有課程、題目與歷史結果不刪除
  而且該老師不能再建立新課程
```
...
> [!note] 規則來源
> [[需求蒐集]] BR-08、BR-16、BR-17、US-02（AC-02-04、AC-02-05）、US-03（AC-03-04、AC-03-05、AC-03-07、AC-03-08）。vote-to-reveal 與逐題控制的狀態規則已由 [[P0 核心需求基線]] 涵蓋，但**題型對應的呈現方式**與**老師端即時彙總視角**未被基線重述，於本檔補齊。問答題呈現是否切換文字列表／文字雲仍為 [[需求蒐集#仍待 M2 或產品細節確認]] 的待確認項。

### Rules

- R-F17-1　學員須先送出自己答案（vote-to-reveal）才能看到該題即時彙總；未作答者於題目 open 時看不到受限制彙總。
- R-F17-2　老師關閉題目時，該題統計自動公布為全班可見（含未投者），無需獨立「公布」操作。
- R-F17-3　Poll 題統計圖為長條圖。
- R-F17-4　Open text 題呈現為文字列表／文字雲（是否可切換待確認，不影響 `open_text` domain schema）。
- R-F17-5　Quiz 題統計圖為長條圖並標示正確答案。
- R-F17-6　題目 open 期間，老師端即時顯示統計圖與已投／加入人數。
- R-F17-7　老師端不看個別學員答案（匿名彙總）。
...
| Web Session 具體 session 儲存與 cookie 值形式？ | M2 | M2 定案 | [[P0 核心需求基線#登入登出與 Web Session]] |
...
| User Story | P0 | 對應 AC | 現行規範 | 效能檔對應 |
|---|---|---|---|---|
| US-F0 老師建立課程 | —（未編 P0） | AC-02-01、BR-01、BR-02、BR-03 | [[需求蒐集#US-02 老師建立課程與互動題目]]、[[P0 核心需求基線#Course 生命週期]] | — |
| US-F1 Course 生命週期（封存與 archived） | P0-01 | P0-01-AC-01、AC-04 | [[P0 核心需求基線#Course 生命週期]] | — |
| US-F2 LiveSession 生命週期 | P0-01 | P0-01-AC-02、AC-03、AC-05、AC-06 | [[P0 核心需求基線#LiveSession 生命週期]] | [[MVP 效能需求 BDD 場景#US-P7 場次逾 8 小時自動關閉（W8）]] |
| US-F3 session code | P0-02 | P0-02-AC-01～AC-03 | [[P0 核心需求基線#Session code]] | — |
| US-F4 題目重用快照 | P0-02 | P0-02-AC-04～AC-06 | [[P0 核心需求基線#題目重用與快照]] | — |
| US-F5 帳號建立與 bootstrap | P0-03 | P0-03-AC-01、AC-02、AC-09 | [[P0 核心需求基線#帳號建立與首位管理員]] | — |
| US-F6 登入登出與 Session | P0-03 | P0-03-AC-03、AC-04、AC-05 | [[P0 核心需求基線#登入登出與 Web Session]] | — |
| US-F7 密碼規則與復原 | P0-03 | P0-03-AC-06、AC-07 | [[P0 核心需求基線#密碼規則與復原]] | — |
| US-F8 帳號停用與 credential 失效 | P0-03 | P0-03-AC-03、AC-08 | [[P0 核心需求基線#角色與帳號狀態]] | — |
| US-F9 題型語意 | P0-04 | P0-04-AC-01、AC-02、AC-07、AC-08 | [[題目領域契約#題型規則]] | — |
| US-F10 validation 與批次 | P0-04 | P0-04-AC-03、AC-04、AC-05 | [[題目領域契約#共用 validation]] | — |
| US-F11 批次預覽確認 | P0-04 | P0-04-AC-05、AC-06 | [[題目領域契約#Web／CLI 批次 command]] | — |
| US-F12 匿名 Participant | P0-05 | P0-05-AC-01、AC-02、AC-08 | [[P0 核心需求基線#匿名 Participant]] | — |
| US-F13 重連恢復狀態 | P0-05 | P0-05-AC-03 | [[P0 核心需求基線#重連]] | [[MVP 效能需求 BDD 場景#US-P3 學員斷線後快速重連恢復狀態（W4）]] |
| US-F14 idempotency 與唯一性 | P0-05 | P0-05-AC-04、AC-05 | [[P0 核心需求基線#Submission 與 idempotency]] | [[MVP 效能需求 BDD 場景#US-P6 網路不穩時安全重送不產生重複答案（W7）]] |
| US-F15 關題競態 | P0-05 | P0-05-AC-06 | [[P0 核心需求基線#Submit／close 競態]] | [[MVP 效能需求 BDD 場景#US-P5 關題與提交競態下依伺服器權威順序判定（W6）]] |
| US-F16 開課授權旗標（管理員視角） | —（未編 P0） | AC-01-02、AC-01-03、BR-01 | [[需求蒐集#角色與帳號模型]]、[[P0 核心需求基線#角色與帳號狀態]] | — |
| US-F17 結果統計圖呈現 | —（未編 P0） | AC-02-04、AC-02-05、AC-03-04、AC-03-05、AC-03-07、AC-03-08、BR-08、BR-16、BR-17 | [[需求蒐集#課程與互動題目機制]] | — |
